...
module.exports = (env, argv) => {
  ...
  // čitam `PATH_PREFIX` i `MODULE_NAME` iz `.env` datoteke
  // > biti za zakačen na `process.env`
  dotenv.config({ path: process.env.WEBPACK_ENV_FILE_PATH ?? '../.env' });

  const { PATH_PREFIX, MODULE_NAME } = process.env;
  const BASE_PATH = `/${PATH_PREFIX}/${MODULE_NAME}`
  ...
  const config = {
    ...
    plugins: [
      ...
      new webpack.DefinePlugin({
        // If the value is a string it will be used as a code fragment -> that's why we need to use JSON.stringify
